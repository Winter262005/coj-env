...
class CloudEnv:
    ...
    def _reset_dev_shutdown(self, rng: random.Random) -> Observation:
        ...
        return Observation(
            instances=[
                Instance(
                    id="i-1",
                    cpu=round(rng.uniform(0.5, 4.5), 1),
                    status="running",
                    tag="dev",
                ),
                Instance(
    ...
    def reset(self, task: str = "auditor") -> dict:
        ...
        self._max_counts = {
            ...
            "insecure": max(
                1,
                sum(1 for db in self._state.databases if db.public),
            ),
        }
    ...
    def step(self, action: dict) -> tuple[dict, float, bool, dict]:
            ...
            for inst in self._state.instances:
                if inst.id == action_obj.target_id:
                    ...
                    if inst.tag == "prod":
                        ...
                        info = {
                            "action_success": True,
                            "reason": "stopped idle dev instance",
                        }
                    else:
                        step_reward -= 0.10
            ...
            for db in self._state.databases:
                if db.id == action_obj.target_id:
                    ...
                    if db.public:
                        ...
                        info = {
                            "action_success": True,
                            "reason": "secured public database",
                        }
                    else:
                        step_reward -= 0.10
        ...
        # Keep within [0,1], then enforce strict (0,1)
        step_reward = max(0.0, min(1.0, step_reward))
        step_reward = _clamp01_open(step_reward)
